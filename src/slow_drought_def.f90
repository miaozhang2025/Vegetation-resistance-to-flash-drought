module sd_def
contains
subroutine slowdrought(n,a,et,b,drt1)
!======================================================================
! References:
   ...
   if(a(j)<thresh)then
      if(flag==0)then                ! drought start
       ...
       if(odur(cnt).ge.8)then
             flag = 0
            if(odur(cnt)<td.or.cnt20.lt.3.or.et0>50.000*odur(cnt).or.tmin.gt.thresh1)then
                    osev(cnt)=0.
                    odur(cnt)=0
                    ospd(cnt)=0.
                    drt(si:j-1)=0
...
do j=2,n
 if(a(j)>0..and.drt(j).eq.0)then
   if(a(j)<thresh)then
      ...
      if(odur1(cnt).ge.8)then !test
           flag = 0
          if(odur1(cnt)<td.or.cnt20.lt.3.or.et0>50.000*odur1(cnt).or.tmin>thresh1)then
                    osev1(cnt)=0.
                    odur1(cnt)=0
                    ospd1(cnt)=0.
                    drt1(si:j-1)=0
         ...
         if(flag2==1)then            ! onset period
            if(tmin<=thresh1)then        ! enter thresh1 (e.g., <20%)
               if(a(j)>=a(j-1))then      ! stop onset period
                  ...
                  if(a(j)<=thresh1)then  ! enter recovery period
                     ...
                     flag=0
                     tmin=99.
                     if(odur1(cnt)<td.or.cnt20<3.or.et0>50.000*odur1(cnt))then
                        osev1(cnt)=0.
                        odur1(cnt)=0
                        ospd1(cnt)=0.
                        drt1(si:j-1)=0
            ...
            if(a(j)<=thresh1)then
               ...
               if(odur1(cnt)<td.or.tmin>thresh1.or.cnt20<3.or.et0>50.000*odur1(cnt))then
                  osev1(cnt)=0.
                  odur1(cnt)=0
                  ospd1(cnt)=0.
                  drt1(si:j-1)=0
...
if(cnt_sd>0)then
   b(1,1)=cnt_sd
   b(2,1:cnt_sd)=real(odur1(1:cnt_sd))!-sum(odur(1:cnt)))/b(1)
   b(3,1:cnt_sd)=real(osev1(1:cnt_sd))!-sum(osev(1:cnt)))/b(1)/b(2)
   b(4,1:cnt_sd)=real(ospd1(1:cnt_sd))
else
   b=0
endif
end subroutine slowdrought 
end module sd_def
